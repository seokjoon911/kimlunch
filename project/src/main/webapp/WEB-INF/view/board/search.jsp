...
<div class="w3-main w3-content w3-padding-" style="max-width:1200px;margin-top:100px">
 ...
 <c:if test="${searchCount > 0}"><tr>
	<c:forEach var="b_board" items="${searchList}" varStatus="stat"><td>
...
<table style="margin:auto; width:100%" >
 <tr align="center"><td colspan="5">
    <div class="w3-bar">
     <c:if test="${pageNum > 1}">
       <a href="search?pageNum=${pageNum - 1}">[이전]</a></c:if>
       <c:if test="${pageNum <= 1}">[이전]</c:if>
     <c:forEach var="a" begin="${startpage }" end="${endpage}">
         <c:if test="${a == pageNum}"><a class="w3-button w3-black" href="#">${a}</a></c:if>
         <c:if test="${a != pageNum}">
           <a class="w3-button w3-hover-black" 
              href="search?pageNum=${a}">${a}</a></c:if>
     </c:forEach>
     <c:if test="${pageNum < maxpage}">
           <a href="search?pageNum=${pageNum + 1}">[다음]</a></c:if>
